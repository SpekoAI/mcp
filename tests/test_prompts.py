...
async def test_prompt_advertised() -> None:
    ...
    assert any(p.name == "migrate_voice_agent" for p in prompts)
...
@pytest.mark.parametrize("platform", ["livekit", "pipecat", "retell", "vapi"])
async def test_migrate_voice_agent_prompt_renders(platform: str) -> None:
    mcp = create_server()
    result = await mcp.render_prompt(
        "migrate_voice_agent",
        {
    ...
    text = "\n".join(m.content.text for m in result.messages)
    assert f"spekoai://docs/migration-{platform}" in text
    assert "speko_migrate" in text
    assert "speko_test" in text
    assert "speko_deploy" in text
    assert "Deploy only after user confirmation" in text or "explicit confirmation" in text


@pytest.mark.parametrize("platform", ["retell", "vapi"])
async def test_migrate_prompt_warns_scaffold_only_adapters(platform: str) -> None:
    mcp = create_server()
    result = await mcp.render_prompt(
        "migrate_voice_agent",
        {"from_platform": platform},
    ...
    text = "\n".join(m.content.text for m in result.messages)
    assert f"@spekoai/adapter-{platform}` is scaffold-only" in text
    assert "Speko SDK/platform APIs" in text
