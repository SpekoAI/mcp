...
# Sibling packages supply public READMEs, while scripts/sync_docs.py also
# fetches docs.speko.dev llms exports into spekoai_mcp/_docs/. Only PUBLIC
# packages are copied in — `packages/core`, `packages/providers`, and the
# root CLAUDE.md are intentionally NOT COPY'd because they contain internal
# architecture details (private packages with `"private": true`, references
# to `apps/*`) we don't want bundled into a publicly-reachable MCP server.
#
