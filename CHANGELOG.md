...
- `phone_numbers.get` and `phone_numbers.list` publish `setupStatus.forwardingSipUri`, the `sip:<E.164>@<host>` address a carrier-imported number must forward inbound calls to. The product asked three times for inbound calling to be pointed at Speko and never printed the address; one customer derived a host from the LiveKit WebSocket domain, wildcard DNS resolved it, and nothing ever reached us to log. Null when the number needs no forwarding, or when the platform has no host configured (#2726).
