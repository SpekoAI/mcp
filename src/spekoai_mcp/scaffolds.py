...
`POST https://api.speko.dev/v1/sessions` via raw fetch (the server SDK
doesn't expose a sessions helper yet — see spekoai://docs/llms-full).
Keeping the raw fetch in the template means the scaffold works with just
...
class ScaffoldFile(BaseModel):
    path: str = Field(description="Project-root-relative path (e.g. app/api/speko/route.ts).")
...
_MULTILINGUAL_APPEND = (
    " Reply in whichever language the caller uses — both English and Spanish are supported."
)
...
def _route_ts(
    ...
    selection = select_ranked(language="en", region=region, optimize_for=optimize_for, limit=1)
    for label, picks in (
        ("STT", selection.stt),
        ("LLM", selection.llm),
        ("TTS", selection.tts),
        ...
        if picks:
            ...
            picks_header_lines.append(
                f"//   {label}: {top.provider_id} "
                f"(score={top.score:.3f}, p50={top.primary_latency_ms}ms)"
            )
        else:
...
# SPEKO_BASE_URL=https://api.speko.dev
"""


def _load_react_voice_session() -> str:
    return (files("spekoai_mcp._components") / "react_voice_session.tsx").read_text(
        encoding="utf-8"
    )
...
def build_voice_app_manifest(
    ...
    prompt = system_prompt if system_prompt is not None else _default_system_prompt(langs)
    ...
    return ScaffoldManifest(
        ...
        install_commands=[
            ...
            "pnpm dlx shadcn@latest add button card label select textarea",
        ],
        env_vars=[
            EnvVar(
                name="SPEKO_API_KEY",
            ...
            EnvVar(
                name="SPEKO_BASE_URL",
                description="Speko API base URL. Default is https://api.speko.dev.",
                required=False,
                example="https://api.speko.dev",
            ),
        ],
        post_install_steps=[
