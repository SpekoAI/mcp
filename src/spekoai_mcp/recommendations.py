...
class PackageRecommendation(BaseModel):
    ...
    required: bool = Field(description="True = core to the scaffold; False = nice-to-have.")
...
class StackRecommendation(BaseModel):
    ...
    intent: dict = Field(
        ...
        description=(
            "Echo of the (language, region, optimize_for) intent that produced the picks."
        ),
    )
    summary: str = Field(
        description=(
...
def _default_packages() -> list[PackageRecommendation]:
    return [
        PackageRecommendation(
            ...
            required=True,
            install_command="pnpm add @spekoai/client",
            docs_uri="spekoai://docs/llms-full",
        ),
        PackageRecommendation(
            name="@spekoai/sdk",
            purpose=(
            ...
            required=True,
            install_command="pnpm add @spekoai/sdk",
            docs_uri="spekoai://docs/llms-full",
        ),
    ]
...
def recommend(
    ...
    """
    selection = select_ranked(language=language, region=region, optimize_for=optimize_for, limit=3)
